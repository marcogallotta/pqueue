...
// Covers both std::string and raw-buffer (Span/MutableSpan) API paths.
// All timing via esp_timer_get_time(). No Serial calls inside timed regions.
// Output: "bench " key=value lines.
...
constexpr std::uint32_t kHeavyBurst  = 300;
constexpr std::uint32_t kHeavyCycles = 2;
#endif
...
static OpResult      gOps[8];
static std::uint32_t gOpCount    = 0;
static MountResult   gMounts[5];
static std::uint32_t gMountCount = 0;
static CompactResult gCompact[2];
static std::uint32_t gCompactCount = 0;
...
#ifndef PQUEUE_BENCH_FAST

static void runRawEnqueue(std::uint32_t payloadB) {
    formatFs();
    pqueue::Queue q(makeConfig(payloadB));
    TEST_ASSERT_TRUE_MESSAGE(q.statsResult().status.ok(), "mount failed");

    std::vector<std::uint8_t> pl(payloadB, 'x');

    std::vector<std::uint64_t> s;
    s.reserve(kEnqN);
    for (std::uint32_t i = 0; i < kEnqN; ++i) {
        const std::int64_t t0 = esp_timer_get_time();
        const auto st = q.enqueue(pqueue::Span(pl.data(), pl.size()));
        s.push_back(static_cast<std::uint64_t>(esp_timer_get_time() - t0));
        TEST_ASSERT_TRUE_MESSAGE(st.ok(), "raw enqueue failed");
        yield();
    }
    printOpStats("raw_enqueue", payloadB, kEnqN, std::move(s));
}

static void runRawPeekPop(std::uint32_t payloadB) {
    formatFs();
    pqueue::Queue q(makeConfig(payloadB));
    TEST_ASSERT_TRUE_MESSAGE(q.statsResult().status.ok(), "mount failed");
    ...
    for (std::uint32_t i = 0; i < kPpN; ++i) {
        TEST_ASSERT_TRUE_MESSAGE(
            q.enqueue(pqueue::Span(pl.data(), pl.size())).ok(), "setup enqueue failed");
        yield();
    }

    std::vector<std::uint64_t> s;
    s.reserve(kPpN);
    std::vector<std::uint8_t> out(payloadB);
    for (std::uint32_t i = 0; i < kPpN; ++i) {
        std::size_t written = 0;
        const std::int64_t t0 = esp_timer_get_time();
        const auto ps = q.peek(pqueue::MutableSpan(out.data(), out.size()), written);
        const auto pp = q.pop();
        s.push_back(static_cast<std::uint64_t>(esp_timer_get_time() - t0));
        TEST_ASSERT_TRUE_MESSAGE(ps.ok(), "raw peek failed");
        TEST_ASSERT_TRUE_MESSAGE(pp.ok(), "pop failed");
        yield();
    }
    printOpStats("raw_peek_pop", payloadB, kPpN, std::move(s));
}

static void runCompactIdleHeavy(std::uint32_t payloadB) {
    formatFs();
    pqueue::Queue q(makeConfig(payloadB));
    TEST_ASSERT_TRUE_MESSAGE(q.statsResult().status.ok(), "mount failed");

    std::vector<std::string> burstPls;
    burstPls.reserve(kHeavyBurst);
    for (std::uint32_t i = 0; i < kHeavyBurst; ++i)
        burstPls.push_back(makePayload(payloadB, i + 1));
    ...
    for (std::uint32_t cy = 0; cy < kHeavyCycles; ++cy) {
        for (std::uint32_t i = 0; i < kHeavyBurst; ++i) {
            TEST_ASSERT_TRUE_MESSAGE(
                q.enqueue(burstPls[i]).ok(), "enqueue failed");
            ++qSz;
        ...
        for (;;) {
            ...
            if (cr.compactions > 0) {
                ...
                stepSamples.push_back(dt);
                Serial.printf(
                    "bench scenario=compact_idle_heavy payload_b=%u cycle=%u step=%u dt_us=%llu\n",
                    payloadB, cy, prodTotal, (unsigned long long)dt);
                Serial.flush();
            } else {
                ++noopTotal;
    ...
    Serial.printf(
        "bench scenario=compact_idle_heavy payload_b=%u summary"
        " burst=%u pop_ratio=%.2f cycles=%u productive=%u noops=%u"
        " p50_us=%llu p90_us=%llu p99_us=%llu max_us=%llu total_us=%llu\n",
        payloadB, kHeavyBurst, (double)kPopRatio, kHeavyCycles, prodTotal, noopTotal,
        (unsigned long long)(hasSamples ? pctile(stepSamples, 50) : 0),
        (unsigned long long)(hasSamples ? pctile(stepSamples, 90) : 0),
        (unsigned long long)(hasSamples ? pctile(stepSamples, 99) : 0),
        (unsigned long long)(hasSamples ? stepSamples.back()      : 0),
...
static void printSummary() {
    ...
    if (gCompactCount > 0) {
        Serial.println();
        Serial.println("  scenario           payload  burst  steps  noops    p50      p90      p99      max");
        Serial.println("  -----------------  -------  -----  -----  -----  ------   ------   ------   ------");
        if (gCompact[0].valid) {
            const auto& c = gCompact[0];
            Serial.printf("  compact_idle        %4uB   %4u  %5u  %5u  %6.0fms  %6.0fms  %6.0fms  %6.0fms\n",
                c.payloadB, kBurst, c.productive, c.noops,
                c.p50 / 1000.0, c.p90 / 1000.0, c.p99 / 1000.0, c.max / 1000.0);
        }
#ifndef PQUEUE_BENCH_FAST
        if (gCompactCount > 1 && gCompact[1].valid) {
            const auto& c = gCompact[1];
            Serial.printf("  compact_idle_heavy  %4uB   %4u  %5u  %5u  %6.0fms  %6.0fms  %6.0fms  %6.0fms\n",
                c.payloadB, kHeavyBurst, c.productive, c.noops,
                c.p50 / 1000.0, c.p90 / 1000.0, c.p99 / 1000.0, c.max / 1000.0);
        }
#endif
    }
...
void test_mount() {
    ...
    runMountCase(50);
    runMountCase(200);
    if constexpr (kMountMax >= 500)  runMountCase(500);
    if constexpr (kMountMax >= 1000) runMountCase(1000);
}

#ifndef PQUEUE_BENCH_FAST
void test_raw_enqueue_256b()       { runRawEnqueue(256); }
void test_raw_peek_pop_256b()      { runRawPeekPop(256); }
void test_compact_idle_heavy_492b() { runCompactIdleHeavy(492); }
#endif
...
#ifndef PQUEUE_BENCH_FAST
    RUN_TEST(test_raw_enqueue_256b);
    RUN_TEST(test_raw_peek_pop_256b);
#endif
    RUN_TEST(test_mount);
    RUN_TEST(test_compact_idle_256b);
#ifndef PQUEUE_BENCH_FAST
    RUN_TEST(test_compact_idle_heavy_492b);
#endif
    UNITY_END();
    printSummary();
}

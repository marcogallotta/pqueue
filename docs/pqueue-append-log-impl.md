...
2. **Rotate-before-compact preflight.** Two booleans determine whether a rotate would fire: `selectedIsLastRange` (the requested range is `manifestRanges_.back()`) and `tailCanMergeWithLastRange` (the active tail is non-empty and its generation equals `back().endGen + 1`). Both must be true for `wouldRotate`. The preflight then computes a hypothetical effective range: if `wouldRotate`, the range extends to include `activeGeneration_` (the tail). Live payload sizes are gathered from `records_` in RAM. A bin-pack loop estimates `hypoOutputSegs`. File sizes for sealed segments are read from disk via `fs()->fileSize()`, with errors propagated immediately; the active tail uses `activeSegmentBytes_` directly. From these, `hypoLiveBytes` and `hypoHasDeadBytes` are derived.
...
5. **Dead-range removal.** If `collectLiveRecords` returns empty, the range is fully dead. A manifest is published that drops the range with no file output, then `cleanupOneDanglingSegment()` is called `inputSegCount - 1` additional times (beyond the one implicit in `publishManifest`) to drain all input segments promptly.

6. **Live-range compaction.** Output segments are written at generations starting from `nextGeneration_`. The replacement manifest replaces the old range with `[firstNewGen, lastNewGen]`, then a two-sided merge pass checks contiguity with both the preceding and following range. After `publishManifest`, `cleanupOneDanglingSegment()` is called `inputSegCount - 1` additional times. `records_` entries for the compacted range are updated in-place to point at the new segment positions.

The extra cleanup passes are necessary because rotate-before-compact seals the former active tail into the compaction input, producing one more dangling input segment than a non-rotate compaction. Without them, the dangling tail inflates `totalOnDiskBytes_` and blocks subsequent writes that check the `maxTotalBytes` limit.

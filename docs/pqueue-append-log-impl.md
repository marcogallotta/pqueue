...
**Active issue: generation gap fragmentation after compaction.** When `compactRange([A,B])` writes output at generations C..D (where C = nextGeneration_ > B), the active segment remains at its current generation (say B-1 if it was open during compaction). When that active segment later rotates, its generation cannot merge with the output range [C,D] because C != (B-1)+1. A separate range is created. The following new segment at generation D+1 cannot merge with the orphaned active range either. This causes the manifest to accumulate 3 ranges after every compaction, permanently triggering range pressure.

The structural fix is to rotate the active segment before compaction writes its output, so its generation is sealed into the compaction input range and the output starts contiguously with whatever follows. Before implementing this, the replay and cleanup invariants must be verified: specifically, whether scanSegments() and cleanupOneDanglingSegment() correctly handle ranges where output generation numbers are numerically higher than the (sealed) active generation that was their logical predecessor. The key supporting invariant is that isLastSegment is determined by manifest position, not numeric generation order -- this is noted in the scan rules above but must be confirmed to hold through the full mount and cleanup paths. See pqueue-compaction-strategy.md for the full analysis.

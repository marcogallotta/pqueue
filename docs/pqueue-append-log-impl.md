...
4. **Rotate (if useful).** If `wouldRotate` and the range has live records, `rotateSegment()` is called. `RangeLimitExceeded` is structurally unreachable here: `rotateSegment()` merges the contiguous tail into the last manifest range before checking the limit, so range count is unchanged. After rotate, the effective range is re-resolved from `manifestRanges_` to pick up the extended endpoint.

5. **Dead-range removal.** If `collectLiveRecords` returns empty, the range is fully dead. A manifest is published that drops the range with no file output, then `cleanupAllDanglingSegments()` is called to remove all input files in one pass.

6. **Live-range compaction.** Output segments are written at generations starting from `nextGeneration_`. The replacement manifest replaces the old range with `[firstNewGen, lastNewGen]`, then a two-sided merge pass checks contiguity with both the preceding and following range. After `publishManifest`, `cleanupAllDanglingSegments()` removes all dangling input segments in one `listFiles` pass. `records_` entries for the compacted range are updated in-place to point at the new segment positions.

...
**Known limitations (v1).** Compaction produces at most one output segment per call; ranges whose live records exceed `maxSegmentBytes` are skipped. `compactFull` may revisit a newly compacted range rather than advancing to the next original range (harmless but inefficient). No multi-range holistic compaction. No repair tooling for corrupted-but-non-fatal states. Cleanup drains one dangling file per publish pass; a store with many accumulated dangling files drains slowly under a quiet write rate.

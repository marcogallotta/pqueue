...
**Known limitations.** `compactFull` may revisit a newly compacted range rather than advancing to the next original range (harmless but inefficient, caused by the newly-created range occupying a low position in the ratio ordering). No multi-range holistic compaction. No repair tooling for corrupted-but-non-fatal states. Cleanup drains one dangling file per publish pass; a store with many accumulated dangling files drains slowly under a quiet write rate.
...
### 1. `maxTotalBytes` / `reservedBytes` is configured but not enforced
